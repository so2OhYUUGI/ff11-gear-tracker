...
export default function AccountStep({
	...
	return (
		<div className="space-y-6">
			<div>
				<h3 className="main-title">Step 1: アカウント設定</h3>
				<p className="page-description">キャラクターを紐付けるアカウントを選んでください</p>
			</div>
			...
			<div className="space-y-4">
					...
					<div className="grid gap-3">
						{accounts.map((acc) => (
							<button
								...
								className={`card text-left border-2 ${!isCreatingNew && selectedAccountId === acc.id ? 'border-blue-500 bg-blue-50/50' : 'border-border'
									}`}
							>
				...
				{/* 新規アカウント作成入力 */}
				<div className={`card ${isCreatingNew ? 'border-blue-500 bg-blue-50/50' : 'border-border'}`}>
					<label className="form-label">新しいゲームアカウントを作成</label>
					<input
						type="text"
						className="form-input mt-2" // w-full is included in form-input
